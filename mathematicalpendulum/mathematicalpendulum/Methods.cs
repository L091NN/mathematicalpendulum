    ...
    class Methods
    {
        ...
        public void Run(DataGridView table)
        {
            ...
            for (int i = 1; i < N;) 
            {
                ...
                sv1 = Math.Abs(h * ((1.0 / 360.0) * k[0] - (128.0 / 4275.0) * k[2] + (127.0 / 6840.0) * k[3] + (1.0 / 50.0) * k[4] + (2.0 / 55.0) * k[5]));
                sv2 = Math.Abs(h * ((1.0 / 360.0) * m[0] - (128.0 / 4275.0) * m[2] + (127.0 / 6840.0) * m[3] + (1.0 / 50.0) * m[4] + (2.0 / 55.0) * m[5]));
                if (!constH)
                {
                    if (sv1 > eps) //|| sv2 > eps)
                    {
                    ...
                    if (sv1 < eps / 128.0) //|| sv2 < eps / 128.0)
                    {
                        h *= 2;
                        doubles++;
                    }
                ...
                curV1 = v1 + h * ((16.0 / 135.0) * k[0] + (6656.0 / 12825.0) * k[2] + (28561.0 / 56430.0) * k[3] - (9.0 / 50.0) * k[4] + (2.0 / 55.0) * k[5]);
                v2 = v2 + h * ((25.0 / 216.0) * m[0] + (1408.0 / 2565.0) * m[2] + (2197.0 / 4104.0) * m[3] - (1.0 / 5.0) * m[4]);
                curV2 = v2 + h * ((16.0 / 135.0) * m[0] + (6656.0 / 12825.0) * m[2] + (28561.0 / 56430.0) * m[3] - (9.0 / 50.0) * m[4] + (2.0 / 55.0) * m[5]);
                v1x.Add(x, curV1);
                v2x.Add(x, curV2);
